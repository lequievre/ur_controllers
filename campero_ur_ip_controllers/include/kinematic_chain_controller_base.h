...
namespace controller_interface
{
	...
	bool KinematicChainControllerBase<T>::init(T *robot, ros::NodeHandle &n)
	{
		...
		for (int i = 0; i < kdl_chain_.getNrOfJoints() && link_; i++)
		{
		    ...
		    #if TRACE_ACTIVATED
				ROS_INFO("KinematicChainControllerBase -> (init) Getting limits for joint: %s, index=%d", joint_->name.c_str(),index);
			#endif
			
			#if TRACE_ACTIVATED
			     if (joint_->limits)
					ROS_INFO("KinematicChainControllerBase -> %s : has limit", joint_->name.c_str());
				 else
				    ROS_INFO("KinematicChainControllerBase -> %s : has no limit", joint_->name.c_str());	
			#endif
		    ...
		    #if TRACE_ACTIVATED
				ROS_INFO_STREAM("KinematicChainControllerBase -> (limits) Getting limits for joint: " << joint_->name.c_str() << ", min=" << joint_limits_.min(index) <<  ", max=" << joint_limits_.max(index));
			#endif
